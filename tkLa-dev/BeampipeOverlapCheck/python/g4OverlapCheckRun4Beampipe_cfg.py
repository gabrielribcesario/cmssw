...
baseName  = 'beampipe' + geomName + 'DDD'
...
process = cms.Process('BeampipeOverlapCheck', ERA)
...
# Check the full hierarchy so every beampipe sub-volume is covered.
process.g4SimHits.G4CheckOverlap.Depth          = cms.int32(-1)

# NodeNames are G4Region names (RegionFlag=True). The three beampipe regions are
# defined in Geometry/TrackerSimData/data/trackerProdCutsBEAM.xml:
#   BeamPipe        -> BeamTube1 (beryllium inner tube)
#   BeamPipeOutside -> all BEAM* envelope volumes (aluminum/stainless tubes)
#   BeamPipeVacuum  -> all BeamVacuum* volumes
...
process.g4SimHits.G4CheckOverlap.NodeNames      = cms.vstring('BeamPipe', 'BeamPipeOutside', 'BeamPipeVacuum')
...
# Set gdml=1 to enable per-volume GDML export restricted to beampipe regions.
